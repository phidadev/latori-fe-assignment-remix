...
export default function CartModal({
    ...
    return (
        ...
        <div className={`${!isVisible ? "hidden ":""}fixed z-30 inset-0 w-full h-full flex justify-center items-center`}>
            ...
            <div className="relative max-w-[85%] md:max-w-xl w-full h-max max-h-[90%] min-h-[40%] md:max-h-[50%] px-3 pb-3 bg-white text-black rounded shadow overflow-scroll flex flex-col">
                <div className="sticky top-0 w-full py-4 border-b border-black flex justify-between items-center gap-8 bg-white">
                    <p className="text-lg">Cart</p>
                    {/* on click use onHide-Function from parent */}
                    <button onClick={onHide} className="text-xl font-light">
                        X
                    </button>
                </div>
                <div className="w-full h-auto py-6 flex flex-col gap-3">
